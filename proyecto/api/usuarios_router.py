...
def listar_usuarios(repo: UsuarioRepository = Depends(get_usuario_repo)):
    return [_a_response(uid, u) for uid, u in repo.listar()]

@router.post("/", response_model=UsuarioResponse, status_code=status.HTTP_201_CREATED)
def crear_usuario(body: UsuarioRequest, repo: UsuarioRepository = Depends(get_usuario_repo)):
    try:
...
def obtener_usuario(usuario_id: int, repo: UsuarioRepository = Depends(get_usuario_repo)):
...
def actualizar_usuario(usuario_id: int, body: UsuarioRequest, repo: UsuarioRepository = Depends(get_usuario_repo)):
    usuario = repo.obtener(usuario_id)
    if not usuario:
        raise HTTPException(status_code=404, detail=f"Usuario {usuario_id} no encontrado.")
...
def eliminar_usuario(usuario_id: int, repo: UsuarioRepository = Depends(get_usuario_repo)):
